...
def post_comment(request, post_pk):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            即根据 post 来过滤该 post 下的全部评论。但既然已经有了一个 Post 模型的实例
            post（它对应的是 Post 在数据库中的一条记录），那么获取和 post 关联的评论列表有一个简单方法，
            即调用它的 xxx_set 属性来获取一个类似于 objects 的模型管理器，然后调用其 all 方法来返回
            这个 post 关联的全部评论。
            '''
